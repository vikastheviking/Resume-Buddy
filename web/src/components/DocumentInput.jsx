...
export default function DocumentInput({ ordinal, label, hint, placeholder, value, onChange, onNotify }) {
  ...
  const ingest = useCallback(
    async (file) => {
      ...
      // slower (rendering + reading the page as an image) - measured over 20s for a
      // single dense page on the actual hosting CPU. Set that expectation up front
      // rather than letting a normal-looking spinner sit there.
      const isPdf = /\.pdf$/i.test(file.name);
      setStatus({
        tone: 'busy',
        message: isPdf
          ? `Extracting text from ${file.name}… scanned documents can take up to 30 seconds.`
